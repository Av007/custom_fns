...
# ---------------------------------------------------------------------------
# ComfyUI node: AspectCrop
#   Center-crops an image to a target aspect ratio and (optionally) resizes
#   it to a target resolution. Useful to force LoadImage outputs to match
#   the aspect ratio expected by a downstream video node (ByteDance, Kling…).
# ---------------------------------------------------------------------------

class AspectCrop:
    """
    Center-crop an IMAGE to the selected aspect ratio.
    Optionally resizes the cropped image to a target resolution (keeping the
    chosen aspect). Pure PyTorch — no PIL/NumPy dependency beyond what
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image":        ("IMAGE",),
                "aspect_ratio": (_ALL_ASPECT_RATIOS, {"default": "16:9"}),
                "resolution":   (["source"] + _ALL_RESOLUTIONS, {"default": "source"}),
            },
        }

    def crop(self, image, aspect_ratio: str, resolution: str):
        import torch  # ComfyUI always ships torch
        import torch.nn.functional as F

        # ComfyUI IMAGE = (B, H, W, C) float tensor in [0,1]
        b, h, w, c = image.shape

        ar_w, ar_h = [float(x) for x in aspect_ratio.split(":")]
        ...
        if resolution != "source":
            dims       = RESOLUTION_DIMS.get(aspect_ratio, RESOLUTION_DIMS["16:9"])
            tgt_w, tgt_h = dims.get(resolution, dims["480p"])
            chw        = cropped.permute(0, 3, 1, 2)
            resized    = F.interpolate(chw, size=(tgt_h, tgt_w), mode="bilinear", align_corners=False)
            cropped    = resized.permute(0, 2, 3, 1).contiguous()

        out_h, out_w = cropped.shape[1], cropped.shape[2]
